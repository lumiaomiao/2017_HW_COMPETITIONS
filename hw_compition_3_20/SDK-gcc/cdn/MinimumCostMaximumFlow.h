...
Edge edge[max_node_num+2][max_node_num+2];
Custome_node custome[500];
int W[max_node_num+2][max_node_num+2];//流量矩阵
int U[max_node_num+2][max_node_num+2],S[max_node_num+2][max_node_num+2];
//剩余网络和费用矩阵
int capacity[max_node_num+2][max_node_num+2],cost[max_node_num+2][max_node_num+2];
int sign[max_node_num+2][max_node_num+2];
...
class MinimumCostMaximumFlow{
	public:
		...
		int m,n;	
	public:
		MinimumCostMaximumFlow(char *filename,vector<int> &server):filename(filename),server(server){}
		virtual ~MinimumCostMaximumFlow(){}
		bool InitializeFW(char * topo[MAX_EDGE_NUM], int line_num);
		void InitializeServer();
		void InitializeUS(int x[max_node_num+2][max_node_num+2]);
...
bool MinimumCostMaximumFlow::InitializeFW(char * topo[MAX_EDGE_NUM], int line_num){
	if(line_num <= 0){
		...
		while(i < line_num){
			...
			custome[tmp_custome].flow = tmp_flow;
			custome[tmp_custome].net_num = tmp_netnode;
...
//根据算法，初始化剩余网络和费用矩阵
void MinimumCostMaximumFlow::InitializeUS(int x[max_node_num+2][max_node_num+2]){
	//step 2,3
...
void MinimumCostMaximumFlow::SearchByMaxCapPath(){
	...
	while(1){
		...
		//cout<<"U[s][t] = "<<U[s][t]<<" S[s][t] = "<<S[s][t]<<endl;
		if(U[s][t] == 0) break;
		else{
			seta = U[s][t];
			l = s;
			...
			while(1){
				W[l][r] -= seta;
				W[r][l] += seta;
				//cout<< l <<" " <<r << " "<<edge[l][r].flow <<endl;
				//若edge[r][l].flow >0 && < seta,则只减去flow这一部分，剩下的在正向方向的flow加进去
				//若flow > seta ,则正向的不加，只在反向的减去即可
...
void MinimumCostMaximumFlow::MinCostMaxFlow(char *topo[MAX_EDGE_NUM], int line_num){
	...
	while(1){
		//建立N关于f的剩余网络的容量矩阵capacity和费用矩阵cost
		ConstructCapCosMat();
		/*cout<<"-----------------------capacity and cost--------------------------------"<<endl;
		for(int row = 0; row < node_num + 2; row++){
			for(int col = 0; col <node_num + 2; col++){
				cout<<"("<<capacity[row][col]<<","<<cost[row][col]<<") ";
			}
			cout<<endl;
			cout<<endl;
		}
...
void MinimumCostMaximumFlow::write(){
	//cout<<"node_num:"<<node_num<<" edge_num:"<<edge_num<<" custome_num:"<<custome_num<<endl;
	/*for(int i = 0; i < node_num + 2; i++){
		true_flow += edge[s][i].flow;
	}*/
	cout<<"需求的流量为："<<request_flow<<endl;
	cout<<"总共的流量为："<<true_flow<<endl;
	cout << "总共的花费为：" << price << endl;
}